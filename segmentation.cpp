...
int main(int argc, char* argv[])//
{
	...
	while (1)
	{
		...
		if (is_first_frame == true) {
			...
			mask_for_circle_copy.create(Original.rows, Original.cols, CV_8UC1);
			mask_for_circle_copy.setTo(Scalar(0, 0, 0));
		...
		last_circu_coord.x=membrane_radius*cos(30.0*float(curSeg-1)*PI/180.0)+membrane_center.x;
		last_circu_coord.y=membrane_radius*sin(30.0*float(curSeg-1)*PI/180.0)+membrane_center.y;

		circu_coord.x=membrane_radius*cos(30.0*float(curSeg)*PI/180.0)+membrane_center.x;
		circu_coord.y=membrane_radius*sin(30.0*float(curSeg)*PI/180.0)+membrane_center.y;
...
*/
		membrane_img_copy = membrane_img.clone();
		Original.copyTo(membrane_img_copy, mask_for_circle_copy);
		...
		if ( frameNumber <= 40) {
			...
			needle_n = "example/" + to_string(frameNumber) + "FFEX.png";
		...
		countP=countNonZero(threshold_needle);
